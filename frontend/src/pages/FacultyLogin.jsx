...
const FacultyLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Invalid Faculty Email or Password.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-tcet-lightBg">
      ...
      <main className="flex-grow flex items-center justify-center p-4 py-16">
        <div className="bg-white border-2 border-tcet-gold w-full max-w-md shadow-xl">
          {/* Card Top Border */}
          <div className="h-2 bg-tcet-gold"></div>
          ...
          <div className="p-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-black text-tcet-navy tracking-tight uppercase">Faculty Login</h2>
              <p className="text-xs text-tcet-mutedText mt-1">Mentor Reviews & Request Approval Dashboard</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>{loading ? 'LOGGING IN...' : 'FACULTY SECURE LOG IN'}</span>
                </button>
              </div>
            </form>

            <div className="mt-6 border-t border-slate-200 pt-4 text-[10px] text-center text-tcet-mutedText leading-relaxed">
              <p>Faculty accounts and assignments are managed by the R&D center coordinator. For credentials recovery, contact cell support.</p>
            </div>
          </div>
        </div>
      </main>
